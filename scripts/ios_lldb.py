...
def connect_command(debugger, command, result, internal_dict):
    ...
    connect_url = g_opts.connect_url
    ...
    g_opts.listener = lldb.SBListener('iosdeploy_listener')

    g_opts.listener.StartListeningForEventClass(
        debugger,
    ...
    process = lldb.target.ConnectRemote(g_opts.listener, connect_url, None, error)
    ...
    while state != lldb.eStateConnected:
        event = lldb.SBEvent()
        if g_opts.listener.WaitForEvent(1, event):
    ...
    for event in events:
        g_opts.listener.AddEvent(event)


def run_command(debugger, command, result, internal_dict):
    global g_opts
    device_app = g_opts.device_app
    ...
    args_arr = args_arr + shlex.split(g_opts.args)

    launchInfo = lldb.SBLaunchInfo(args_arr)
    launchInfo.SetListener(g_opts.listener)
...
def autoexit_command(debugger, command, result, internal_dict):
    ...
    detectDeadlockTimeout = g_opts.detect_deadlock_timeout
    ...
    while True:
        if g_opts.listener.WaitForEvent(1, event) and lldb.SBProcess.EventIsProcessEvent(event):
        ...
        if state == lldb.eStateExited:
            ...
            os._exit(g_opts.exitcode_app_crash)
